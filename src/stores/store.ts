    ...
    (set, get) => ({
      ...
      register: async (data) => {
        ...
        try {
          ...
          set({ user: res.data.data, token: res.data.token ?? null, loading: false });
        } catch (err) {
      ...
      logout: async () => {
        ...
        if (user && token) {
          try {
            await axios.post(`${API_BASE_URL}/api/logout/${user.id}`, {}, { headers: { Authorization: `Bearer ${token}` } });
          } catch (err) {
            console.error("Logout API failed", err);
          }
        }
        set({ user: null, token: null });
        ["auth-store", "organizers-store", "followers-store"].forEach((key) => localStorage.removeItem(key));
      ...
      updateAccount: async (data) => {
        ...
        try {
          const response = await axios.put<AuthResponse>(`${API_BASE_URL}/api/update-account/${user.id}`, data, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (response.data.success) set({ user: response.data.data, token: response.data.token ?? token });
          else console.log(response.data.message || "Update failed");
        } catch (err) {
          console.error(err);
